...
def require_clean_check_timing(path: Path, expected_design: str | None = None,
                               expected_version: str | None = None) -> None:
    text = stable_read(path).decode("utf-8", errors="strict")
    blockers = ("no_clock", "no_input_delay", "no_output_delay", "unconstrained",
                "no_drive", "no_load")
    for name in blockers:
        if re.search(rf"(?i)(?<![A-Za-z0-9_]){name}(?![A-Za-z0-9_])", text):
            raise CohortError(f"check_timing contains {name} endpoints")
    if text.count("TIMING CHECK SUMMARY") != 1 or \
            text.count("TIMING CHECK IDEAL CLOCKS") != 1 or \
            "TIMING CHECK DETAIL" in text or \
            not re.search(r"(?m)^#  Generated by:\s+Cadence Innovus\s+\S+\s*$", text) or \
            not re.search(r"(?m)^#  Design:\s+\S+\s*$", text) or \
            not re.search(r"(?m)^#  Command:\s+check_timing -verbose\s*$", text):
        raise CohortError("check_timing report lacks exact native summary form")
    if expected_design is not None and not re.search(
            rf"(?m)^#  Design:\s+{re.escape(expected_design)}\s*$", text):
        raise CohortError("check_timing design identity mismatch")
    if expected_version is not None and not re.search(
            rf"(?m)^#  Generated by:\s+Cadence Innovus\s+"
            rf"{re.escape(expected_version)}\s*$", text):
        raise CohortError("check_timing tool identity mismatch")
    warning_rows = re.findall(
        r"(?m)^\s*\|\s*([a-z][a-z0-9_]*)\s*\|[^|\n]*\|\s*([0-9]+)\s*\|\s*$",
        text)
    if warning_rows != [("ideal_clock_waveform", "1")] or not re.search(
            r"(?m)^\s*\|\s*core_clk\s*\|\s*core_setup_view\s*\|\s*$", text):
        raise CohortError("check_timing warning inventory is not exact clean core form")
...
def execute_stage(root: Path, candidate: str, stage: str, authorization: str) -> Path:
    ...
    if stage == "genus":
        ...
        require_clean_check_timing(
            output / "reports/check_timing.rpt", descriptor["top"], tool["version"])
        names = [
            ...
            "reports/drc_pre_signal_eco.rpt", "reports/drc.rpt",
            "reports/antenna.rpt", "reports/route.rpt",
...
def validate_execution_receipt(path: Path, root: Path, candidate: str,
    ...
    if stage == "genus":
        ...
        require_clean_check_timing(
            output / "reports/check_timing.rpt", document["top"],
            contract["tools"]["innovus"]["version"])
    return document
