...
def validate_final_registry_document(document: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_pointer = {
        ...
        "sha256": "4ffda1982bdc3925723cd22821d06b27a273c9e9e4acc05db6150c9fe84d7d9d",
        "source_commit": "ba0116029bb79573dca23c3957845885837f4b82",
        "publication_commit": "823b768ba3dad82b3de0febd3d5f2c556c0643be",
    }
...
def tool_identity(path: Path) -> dict[str, Any]:
    ...
    versions.update(re.findall(
        r"(?mi)^\s*TOOL:\s*xrun(?:\(64\))?\s+"
        r"(\d{2}\.\d{2}-s\d+(?:_\d+)?)(?![A-Za-z0-9_.-])\s*$",
        version.stdout))
