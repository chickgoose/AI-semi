...
RTL_COMMIT = "4ce4836fab1309d3468db8e660d2da9af371f784"
RTL_ROWS = {
    "a2": {
        ...
        "sources": [
            ...
            ("rtl/technology/single_edge/w2_single_edge_pair_tx.sv", "05d0cb2bc7b84e585d66771f10a447cd14cfc1d8c41cf6a02c5419f6bac60493"),
            ("rtl/technology/single_edge/w2_single_edge_pair_rx.sv", "c6ebefc560e158d4ffa4d1ac340c1c1b65d8caafbe2c1a8957fadbea3b7e59a5"),
            ("rtl/technology/single_edge/w2_single_edge_exact_pair_endpoint.sv", "b4af9db6f4aebdedd150e53dda0fc4737c9c8c4450ecf1ab51a1625685e07ea4"),
            ("rtl/candidates/a2_batched_iwrr_single_edge/a2_batched_iwrr_single_edge_top.sv", "9061deb14e46e0d050bdf3fae450ee8a94c234f94e74f69e1dc8be84722e55ac"),
        ],
    },
    "a3": {
        ...
        "sources": [
            ...
            ("rtl/technology/single_edge/w2_single_edge_pair_rx.sv", "c6ebefc560e158d4ffa4d1ac340c1c1b65d8caafbe2c1a8957fadbea3b7e59a5"),
            ("rtl/technology/single_edge/w2_single_edge_exact_pair_endpoint.sv", "b4af9db6f4aebdedd150e53dda0fc4737c9c8c4450ecf1ab51a1625685e07ea4"),
            ("rtl/candidates/a3_exact_scalar_prefix_k2_single_edge/a3_exact_scalar_prefix_k2_single_edge_top.sv", "3bac4faa2e249e0326a8eba4e3b010437e16c33b5dff236bd621acdb27c5bb07"),
        ],
    },
}
GENERIC_FILELIST = {
    "path": "rtl/technology/single_edge/filelists/generic.f",
    "sha256": "8c538fc9147c96237f5367fa25da91e2f787ee67d09ca6738f5fcf7e9f19ddf8",
    "entries": [
...
def validate_contract() -> tuple[bytes, dict[str, Any]]:
    ...
    if contract.get("rtl_authority", {}).get("commit") != RTL_COMMIT:
    ...
    for name in contract["candidate_order"]:
        ...
        if len(expected_sources) != 5 or len({item["path"] for item in expected_sources}) != 5:
    ...
    if constraints.get("authority_status") != "UNCONFIRMED_TEAM_PLACEHOLDER" or \
            ...
            constraints.get("candidate_go_eligible") is not False:
    ...
    if qualification.get("maximum_decision_under_this_contract") != \
            "HOLD_PLACEHOLDER_CONSTRAINT_AUTHORITY" or \
    ...
    sdc = stable_read(repo_path(constraints["sdc"])).decode("utf-8")
    required_sdc = (
    ...
    mandatory = {"setup_timing", "hold_timing", "postroute_area", "drc", "antenna",
...
def package_inventory(contract: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    paths.extend(Path(path) for path in contract["flow_templates"].values())
...
def static_preflight(output: Path | None) -> dict[str, Any]:
    ...
    document = seal({
        ...
        "real_tools_or_pdk_examined": False,
        "real_artifacts_examined": False,
        "candidate_physical_go_allowed": False,
        "maximum_decision": "HOLD_PLACEHOLDER_CONSTRAINT_AUTHORITY",
    })
...
def planned_commands(contract: dict[str, Any], design: str,
    ...
    innovus_env = {
        ...
        "SE_MMMC": str(repo_path(contract["flow_templates"]["innovus_mmmc"])),
...
def capture_environment(pdk_root: Path, genus: Path, innovus: Path,
    ...
    names = ("HOME", "USER", "LOGNAME", "SHELL", "LANG", "LC_ALL", "PATH",
             "LD_LIBRARY_PATH", "CDS_LIC_FILE", "LM_LICENSE_FILE", "CDS_AUTO_64BIT")
    ...
    document = seal({
        ...
        "real_server_bytes_verified": True, "candidate_physical_go_allowed": False,
    })
    write_exclusive(output, document)
    return document
...
def execute_stage(design: str, stage: str, plan_path: Path,
    ...
    environment_payload, environment = validate_real_environment(
    ...
    if stage == "innovus":
        ...
        _, genus_receipt = load_json(genus_receipt_path, "Genus execution receipt")
        verify_seal(genus_receipt, "Genus execution receipt")
        if (genus_receipt.get("schema") != "k2_single_edge_execution_receipt_v1" or
                genus_receipt.get("stage") != "genus" or
                genus_receipt.get("design") != design or
                genus_receipt.get("status") != "PASS_NATIVE_EXIT_ZERO" or
                genus_receipt.get("contract_sha256") != contract_sha or
                genus_receipt.get("command_plan_sha256") != sha256(plan_payload)):
            raise FlowError("Innovus is not bound to a successful Genus execution")
    ...
    receipt = seal({
        "schema": "k2_single_edge_execution_receipt_v1", "design": design,
        "stage": stage,
        ...
        "real_environment_receipt_sha256": sha256(environment_payload),
...
def safe_artifact(root: Path, raw: str) -> Path:
    ...
    try:
        ...
        raise FlowError(f"artifact escapes attempt root: {raw}") from error
    return path
...
def require_zero_native(payload: bytes, kind: str) -> None:
    ...
    if re.search(rf"(?i){kind}[^\n]*(?:violations?|errors?)\s*[:=]\s*[1-9][0-9]*", text):
        raise FlowError(f"{kind} report contains violations")


def validate_real_environment(path: Path, contract_sha: str,
                              contract: dict[str, Any]) -> tuple[bytes, dict[str, Any]]:
    payload, document = load_json(path, "real environment receipt")
    verify_seal(document, "real environment receipt")
    if document.get("schema") != "k2_single_edge_real_environment_v1" or \
            document.get("contract_sha256") != contract_sha or \
            document.get("real_server_bytes_verified") is not True:
        raise FlowError("real environment receipt contract mismatch")
    for name, expected in contract["tools"].items():
        ...
        resolved = Path(observed["entrypoint"]).resolve(strict=True)
        if str(resolved) != expected["resolved_path"] or \
                sha256(stable_read(resolved, allow_tool_symlink=True)) != expected["sha256"]:
            raise FlowError(f"live {name} bytes no longer match receipt")
    for role, expected in contract["technology"]["files"].items():
        row = document.get("technology", {}).get(role, {})
        live = stable_read(Path(row.get("path", "")))
        if row.get("sha256") != expected["sha256"] or sha256(live) != expected["sha256"]:
            raise FlowError(f"live technology bytes no longer match: {role}")
    env = document.get("environment_allowlist")
    if not isinstance(env, dict) or sha256(canonical(env)) != \
...
def expected_artifact_paths(top: str) -> dict[str, str]:
    return {
        ...
        "innovus_database_manifest": f"innovus/database/{top}.enc",
...
def build_ledger(design: str, attempt_root: Path, plan_path: Path,
    ...
    if design not in contract["candidate_order"]:
        raise FlowError("unknown candidate")
    _, plan = validate_plan(plan_path, design, sha256(contract_payload), contract)
    ...
    genus_roles = {role for role, path in paths.items() if path.startswith("genus/")}
    rows = []
    for role in contract["artifact_ledger"]["required_roles"]:
        raw = paths[role]
        payload = stable_read(safe_artifact(attempt_root.resolve(), raw))
        rows.append({
            "role": role, "path": raw, "sha256": sha256(payload),
            "size_bytes": len(payload),
            "producer_command_sha256": command_hashes[
    ...
    document = seal({
        ...
        "command_plan_sha256": sha256(stable_read(plan_path)),
...
def validate_artifacts(root: Path, ledger_path: Path, design: str,
    ...
    if ledger.get("schema") != contract["artifact_ledger"]["schema"] or \
            ...
            contract["candidates"][design]["top"]:
    ...
    command_hashes = {row["command_sha256"] for row in plan["commands"]}
    ...
    for role in required:
        ...
        if not SHA256.fullmatch(str(row["sha256"])) or \
                row["producer_command_sha256"] not in command_hashes:
    ...
    if BAD_LOG.search(innovus_log) or \
            not re.search(rf"(?m)^Version:\s+v?{re.escape(contract['tools']['innovus']['version'])}", innovus_log) or \
            f"K2_SINGLE_EDGE_INNOVUS_COMMANDS_COMPLETE top={top}" not in innovus_log or \
            not re.search(r"Message Summary:.*0 error\(s\)", innovus_log) or \
            not re.search(r'--- Ending "Innovus".*---\s*\Z', innovus_log):
        raise FlowError("Innovus log lacks native version/clean completion evidence")
    ...
    blockers = ("no_clock", "no_input_delay", "no_output_delay", "unconstrained", "no_drive", "no_load")
    for blocker in blockers:
        if re.search(rf"(?i)\b{blocker}\b[^\n]*[:|=]\s*[1-9][0-9]*", check_timing):
            raise FlowError(f"check_timing reports {blocker} endpoints")
    if "TIMING CHECK SUMMARY" not in check_timing:
        raise FlowError("check_timing lacks native summary")
    mapped_sdc = artifacts["mapped_sdc"].decode("utf-8")
    if len(re.findall(r"(?m)^\s*create_clock\b", mapped_sdc)) != 1 or \
            re.search(r"(?m)^\s*create_generated_clock\b", mapped_sdc) or \
            "clk_i" not in mapped_sdc or any(command not in mapped_sdc for command in
                ("set_input_delay", "set_input_transition", "set_output_delay", "set_load")):
        raise FlowError("mapped SDC lacks the single-clock and complete I/O/load constraints")
    for role in ("mapped_netlist", "postroute_netlist"):
        text = artifacts[role].decode("utf-8")
    ...
    area_text = artifacts["postroute_area"].decode("utf-8")
    areas = [float(value) for value in re.findall(
        r"(?i)(?:total\s+cell\s+area|cell\s+area)\s*[:=]\s*([0-9]+(?:\.[0-9]+)?)", area_text)]
    if not areas or not all(value > 0 and math.isfinite(value) for value in areas):
...
def hold_receipt(contract_sha: str, design: str, blockers: Iterable[str]) -> dict[str, Any]:
    return seal({
        "schema": "k2_single_edge_physical_qualification_v1", "design": design,
        "decision": "HOLD_MISSING_REAL_ARTIFACTS", "contract_sha256": contract_sha,
        "blockers": list(blockers), "real_artifacts_verified": False,
...
def qualify(design: str, attempt_root: Path, environment: Path, plan_path: Path,
    ...
    if design not in contract["candidate_order"]:
        raise FlowError("unknown candidate")
    contract_sha = sha256(contract_payload)
    missing = [str(path) for path in (environment, plan_path, ledger_path) if not path.exists()]
    if missing:
        document = hold_receipt(contract_sha, design, missing)
        write_exclusive(output, document)
        return document
    env_payload, _ = validate_real_environment(environment, contract_sha, contract)
    plan_payload, plan = validate_plan(plan_path, design, contract_sha, contract)
    ledger_payload, _, metrics = validate_artifacts(
        attempt_root.resolve(), ledger_path, design, contract, plan)
    document = seal({
        "schema": "k2_single_edge_physical_qualification_v1", "design": design,
        "decision": "HOLD_PLACEHOLDER_CONSTRAINT_AUTHORITY",
        "contract_sha256": contract_sha,
        "environment_receipt_sha256": sha256(env_payload),
        "command_plan_sha256": sha256(plan_payload),
        "artifact_ledger_sha256": sha256(ledger_payload),
        "verified_metrics": metrics, "real_artifacts_verified": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "static":
            ...
            print(f"K2_SINGLE_EDGE_REAL_ENV_PASS sha256={receipt['document_sha256']}")
