...
def require_innovus_header(raw: str, top: str, version: str,
    ...
    if generators != [version] or designs != [top] or len(commands) != 1 or \
            re.match(rf"^{re.escape(command)}(?:\s|$)", commands[0]) is None:
...
def require_zero_native(payload: bytes, kind: str, top: str,
    ...
    command = {
        "drc": "verify_drc", "antenna": "verify_process_antenna",
        "connectivity": "verifyConnectivity -type all",
        "pg_connectivity": "verifyConnectivity -type special",
    }[kind]
...
def validate_check_timing(payload: bytes, top: str, version: str) -> None:
    ...
    require_innovus_header(raw, top, version, "check_timing", "check_timing")
...
def validate_innovus_timing(payload: bytes, top: str, version: str,
    ...
    require_innovus_header(raw, top, version, "report_timing", label)
...
def validate_innovus_area(payload: bytes, top: str, version: str) -> float:
    ...
    require_innovus_header(raw, top, version, "report_area", "postroute area")
