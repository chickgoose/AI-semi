...
OUT_ROOT="${AER_CLEAN_OUT:-$PROJECT_ROOT/results/a6-lossless-codec}"
...
if [[ -z "$SIMULATOR" ]]; then
  if command -v xrun >/dev/null 2>&1; then
    ...
    printf 'no supported simulator; set AER_SIMULATOR and AER_IVERILOG/AER_VVP\n' >&2
    exit 1
  fi
fi
...
case "$SIMULATOR" in
  xrun)
    snapshot=aer_clean_a6_lossless_codec_n16
    (cd "$PROJECT_ROOT" && xrun -64bit -sv -timescale 1ns/1ps \
      ...
      -f rtl/candidates/a6_lossless_aer_codec/a6_candidate_tb.f \
    ...
    for test_name in "${tests[@]}"; do
      (cd "$PROJECT_ROOT" && xrun -64bit -R -snapshot "$snapshot" \
        ...
        "+CANDIDATE=a6-lossless-codec" "+METRICS=$out_dir/$test_name.csv" \
  ...
  iverilog)
    ...
    for test_name in "${tests[@]}"; do
      "${runtime[@]}" "$out_dir/aer_clean.vvp" "+CLEAN_TEST=$test_name" \
        "+CANDIDATE=a6-lossless-codec" "+METRICS=$out_dir/$test_name.csv" \
        "+EVENT_METRICS=$out_dir/$test_name.events.csv" \
        "+STIM_CYCLES=$STIM_CYCLES" "+LOAD_PCT=$LOAD_PCT" "+SEED=$SEED" \
  ...
  verilator)
    ...
    (cd "$PROJECT_ROOT" && "${verilator_env[@]}" "$VERILATOR" --binary \
      ...
      -f rtl/candidates/a6_lossless_aer_codec/a6_candidate_tb.f \
      --Mdir "$obj_dir" -o a6-clean-sim)
    for test_name in "${tests[@]}"; do
      "$obj_dir/a6-clean-sim" "+CLEAN_TEST=$test_name" \
        "+CANDIDATE=a6-lossless-codec" "+METRICS=$out_dir/$test_name.csv" \
        "+EVENT_METRICS=$out_dir/$test_name.events.csv" \
        "+STIM_CYCLES=$STIM_CYCLES" "+LOAD_PCT=$LOAD_PCT" "+SEED=$SEED" \
        "${trace_args[@]}" | tee "$out_dir/$test_name.log"
...
printf 'A6 lossless codec benchmark complete: %s\n' "$out_dir"
