...
# Fail-closed orchestration shared by the official 22-trace runners.
...
if len(paths) != len(expected) or duplicates or missing or unexpected or errors:
    details = [
        ...
        f"duplicates={duplicates}", f"missing={missing}",
        f"unexpected={unexpected}", *errors,
    ]
...
pairwise_cross_map_require_reports() {
  local identity_report="$1" affine_report="$2" scope="$3"
...
pairwise_cross_map_compare() {
  local project_root="$1" expected_candidate="$2" freshness_marker="$3"
  local identity_manifest="$4" identity_report="$5"
  local affine_manifest="$6" affine_report="$7" output="$8"
  local output_dir temporary_output validation_status input
  ...
  # Validate report scope before allocating a temporary path, so failed
  # candidate checks cannot leave temporary artifacts behind.
  python3 -c '
import json, pathlib, sys
expected = sys.argv[1]
for label, name in (("identity", sys.argv[2]), ("affine", sys.argv[3])):
    try:
        candidate = json.loads(pathlib.Path(name).read_text(encoding="utf-8")).get("candidate")
    except (OSError, json.JSONDecodeError) as exc:
        raise SystemExit(f"{label} pairwise report cannot be read: {exc}")
    if candidate != expected:
        raise SystemExit(f"{label} pairwise candidate mismatch: {candidate!r} != {expected!r}")
' "$expected_candidate" "$identity_report" "$affine_report" || return 2
  ...
  if ! python3 \
    ...
    --affine-report "$affine_report" --output "$temporary_output"; then
...
if not isinstance(payload.get("rankable"), bool):
...
raise SystemExit(0 if payload["rankable"] else 3)
...
print("PAIRWISE_CROSS_MAP_NON_RANKABLE " + ",".join(payload["rankability_reasons"]), file=sys.stderr)
' "$output"
    return 3
  fi
  return 0
