...
usage() {
  printf '%s\n' \
    'usage: run_ganghee_cluster2_benchmark.sh [sink-always-ready core tests ...]' \
    '' \
    'Required environment:' \
    '  AER_GANGHEE_CLUSTER2_TOP       native cluster2 module name' \
    '  AER_GANGHEE_CLUSTER2_RTL       native RTL source path' \
    '    or' \
    '  AER_GANGHEE_CLUSTER2_FILELIST  native RTL file-list path' \
    '' \
...
[[ "$TOP" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || {
  printf 'invalid AER_GANGHEE_CLUSTER2_TOP=%s\n' "$TOP" >&2
  exit 2
}
if [[ -n "$RTL" && -n "$NATIVE_FILELIST" ]]; then
  printf 'set only one of AER_GANGHEE_CLUSTER2_RTL or AER_GANGHEE_CLUSTER2_FILELIST\n' >&2
  exit 2
fi
...
if [[ -n "$TRACE_JSONL" || -n "$TRACE_MANIFEST" ]]; then
  ...
  for test_name in "${tests[@]}"; do
    is_core_test "$test_name" || {
      printf 'unsupported cluster2-binding test: %s (sink-always-ready core only)\n' \
        "$test_name" >&2
      exit 2
    }
  done
...
command -v xrun >/dev/null 2>&1 || {
  printf 'Ganghee cluster2 binding qualification requires Xcelium xrun\n' >&2
  exit 1
}

out_dir="$OUT_ROOT/ganghee-cluster2-n16-seed${SEED}"
mkdir -p "$out_dir"
...
if [[ -n "$TRACE_JSONL" ]]; then
  ...
  [[ "$trace_sink_mode" == 0 ]] || {
    printf 'Ganghee cluster2 binding supports sink-always-ready traces only\n' >&2
    exit 2
  }
  trace_args=("+TRACE_FILE=$prepared_trace" "+TRACE_NAME=$trace_report_name")
fi

snapshot=aer_clean_ganghee_cluster2_n16
...
for test_name in "${tests[@]}"; do
  run_command=(xrun -64bit -R -snapshot "$snapshot"
    -xmlibdirname "$out_dir/xcelium.d"
    "+CLEAN_TEST=$test_name" "+CANDIDATE=ganghee-cluster2-row-bitmap"
...
printf 'Ganghee cluster2 clean benchmark complete: %s\n' "$out_dir"
