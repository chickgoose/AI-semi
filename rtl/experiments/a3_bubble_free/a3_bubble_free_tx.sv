module a3_bubble_free_tx #(
  ...
  // An occupied TX can be refilled on the same edge that its current event
  // transfers to the RX. When the RX stalls, ready goes low and all TX state
  // remains unchanged.
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (accept_event) begin
        // This branch has priority when completion and refill coincide.
        full_q   <= 1'b1;
        addr_q   <= event_addr_i;
