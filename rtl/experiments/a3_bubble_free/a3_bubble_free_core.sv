module a3_bubble_free_core #(
  ...
  // Reuse the baseline arbiter unchanged so TX refill is the only experiment
  // variable. Source zero remains the highest-priority source.
  ...
  // Keep the baseline elastic RX unchanged for a controlled comparison.
  aer_rx #(
    .ADDR_WIDTH(ADDR_WIDTH),
    .SOURCE_INDEX_WIDTH(SOURCE_INDEX_WIDTH)
  ) u_rx (
