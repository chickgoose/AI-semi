...
module w2_r1_candidate_endpoint_tech (
  ...
  a7_r1_launch_qualifier launch_qualifier (
    .ref_clk_i, .rst_n, .event_valid_i, .event_ready_o,
    .launch_fire_o(launch_fire)
  );
  w2_r1_ddr_tx_tech tx (
    .ref_clk_i, .sample_clk_i, .rst_n, .launch_fire_i(launch_fire), .event_addr_i,
    .frame_active_o(frame_active), .burst_clk_o, .burst_data_o
  );
  w2_r1_ddr_rx_tech rx (
    .rst_n, .burst_clk_i(burst_clk_o), .burst_data_i(burst_data_o),
    .retire_addr_o(raw_retire_addr), .retire_toggle_o(raw_retire_toggle)
  );
  a7_r1_retire_observer retire_observer (
    .ref_clk_i, .rst_n, .raw_addr_i(raw_retire_addr),
    .raw_toggle_i(raw_retire_toggle), .retire_addr_o, .retire_valid_o,
    .seen_toggle_o(seen_retire_toggle)
  );

  assign drain_idle_o = ~launch_fire & ~frame_active & ~burst_clk_o &
                        ~(raw_retire_toggle ^ seen_retire_toggle) & ~retire_valid_o;
endmodule
