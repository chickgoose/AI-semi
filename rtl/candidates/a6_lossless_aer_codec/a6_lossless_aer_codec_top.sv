`timescale 1ns/1ps
module a6_lossless_aer_codec_top #(
  ...
  initial begin
    if (NUM_SOURCES != (1 << SOURCE_WIDTH))
      $error("A6 codec requires a power-of-two NUM_SOURCES");
    if (EVENT_WIDTH < SOURCE_WIDTH + 2)
      $error("A6 frozen binding requires source plus fixed polarity/type bits");
  end
  ...
  a6_codec_encoder #(
    .ADDRESS_WIDTH(SOURCE_WIDTH)
  ) encoder (
    .clk(clk),
    .rst_n(rst_n),
    .event_valid(selected_valid),
    .event_ready(encoder_ready),
    .event_address(selected_source),
  ...
  a6_codec_decoder #(
    ...
    .link_data(link_data),
    .link_ready(link_ready),
    .event_valid(decoder_valid),
    .event_address(decoder_source),
    .event_ready(retire_ready)
  );

  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      rr_start <= '0;
    end else if (selected_valid && encoder_ready) begin
