...
> 교육용 GPDK045 범위 안에서 A2를 주후보·A3를 exact-prefix fallback으로 한 16-source complete single-edge AER endpoint를 구현하고, 동일한 팀 정의 traffic에서 accepted-event exact-once·source overrun·throughput·latency와 complete-boundary post-route timing·area·mapped vectorless power를 입증한다. P6/multi-edge는 superseded/noncurrent 연구 비교로만 보존하고, known-motion world↔sensor 변환은 endpoint PPA 밖의 system demo로 검증한다.
...
- Hardened single-edge actual RTL과 producer-native campaign은 팀 canonical
  synthetic 및 public projected extension의 scoped digital 범위에서 PASS다.
  이것은 mapped physical/vectorless, 최종 후보 선택 또는 team release PASS가
  아니다.
...
1. `CANONICAL_DIGITAL`: 동일 full50 trace와 TB/window/tool로 A2/A3 actual RTL을 실행하고 accepted-event exact-once와 loss conservation을 event 단위로 확인했다. 현재 판정은 `PASS_SCOPED_NATIVE_CAMPAIGN`이다.
2. `INTERFACE`: single-edge `PARALLEL_FALLBACK`만 현재 release-eligible interface로 선택했다. P6는 이 gate에 들어오지 않으며, 독립적인 mapped P&R/power와 최종 선택 전까지 interface release는 HOLD다.
...
1. 닫힌 A2/A3 native campaign publication과 nonrelease claim boundary를 보존한다.
2. 주최 측에 허용 cell/clock 구조와 정확한 공통 PDK corner/clock/I/O 표를 서면 확인한다.
3. P6는 noncurrent 역사 자료로만 유지하고, 선택된 single-edge interface의 A2/A3를 동일 조건 complete-boundary P&R/vectorless/CDC로 비교한다.
...
- 현재 native campaign 회귀 112개와 별도 adversarial 보안 회귀 13개가 모두 PASS했다.
...
- 이 single-edge 결과와 native publication은 scoped canonical digital PASS다.
