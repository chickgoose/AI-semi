# Cluster2 AER → World-frame Direction Ray → World Grid 확장성 브리핑
...
> timestamp와 pose를 쓰는 팀 정의 world-frame direction-ray software path에서
> 8,420개 WORLD ray를 512×256 software grid로 mapping했다. 이것은 기능 확장 경로의
> scoped feasibility 증거이며 외부 CAV 시스템 연동, wire-complete converter
> RTL, latency-quality 또는 converter/world PPA 증거가 아니다.

> **용어 주의:** 기존 파일명·schema·필드에 남아 있는 `CAV`는 외부 표준이나
> 기존 시스템을 뜻하지 않는다. event와 occurrence-time pose를 결합해 world
> frame의 방향 ray로 투영하는 팀 내부 software 경로의 역사적 식별자다.
> 이 결과는 depth·translation을 포함한 metric 3D world point 변환이 아니다.

## 1. 문제 정의

`cluster2_steal_buf`는 16개 source의 event를 source당 depth-2 buffer에
받고, 최대 두 row/bitmap lane으로 retire하는 native AER다. 하지만
native 출력을 pose-aware world-frame direction ray와 grid로 확장하려면
다음 문제를 분리해야 한다.

1. native AER에 없는 event identity와 원본 UZH timestamp를 어떻게
   손실 없이 다시 연결할 것인가?
2. UZH sensor time, 1 ms workload bin, software projection cycle, native retire cycle을
   섞지 않고 어떻게 관리할 것인가?
3. native scheduling latency가 geometry를 바꾸지 않은 채, occurrence-time
   world projection과 world-grid 경로를 재사용할 수 있는가?

이 1차 결과의 질문은 **world-frame direction-ray downstream으로의 확장 가능성**이다.
외부 CAV 시스템과의 연계나 AER scheduling에 따른 투영 정확도·영상 품질 개선은
측정하지 않았다.
...
official UZH shapes_rotation
 ...
 pinned cluster2_steal_buf + observational TB
      ...
      shared occurrence-time neutral input
           /                |                \
     RAW-WORLD        AER-OCC-WORLD      AER-RET-WORLD
           \                |                /
          same software world-projection geometry
                    |
...
object를 공유한다. 기존 증거의 `AER-RET-CAV` view만
`(retire_cycle,event_id)` sidecar presentation order와 native retire
cycle/latency를 보존한다. native
...
### Software world-frame direction-ray projection and world-grid replay
...
| occurrence-time WORLD rays | 8,420 |
...
기존 증거 식별자인 `RAW-CAV`, `AER-OCC-CAV`, `AER-RET-CAV`의 geometry digest는 모두
...
서명된 제3자 attestation이나 hardware converter RTL/PPA/performance 증거가 아니다.
...
| --- | --- | --- | --- |
| `event_timestamp_ns` | 원본 UZH sensor timestamp, ns | geometry order, pose lookup, neutral projection input | native retire time으로 대체 |
...
| `cav_occurrence_cycle` | 기존 schema 이름; `ceil((event_timestamp_ns-window_start_ns)*1000/6500)`인 독립 6.5 ns software projection cycle | strict-past pose visibility, occurrence-time world projection | native occurrence/retire cycle과 혼용 |
| `retire_cycle` | 2 ns Xcelium native clock에서 관측한 retirement cycle index | `(retire_cycle,event_id)` sidecar order, latency | world-projection geometry의 pose-lookup time으로 사용 |
...
| **PASS** | 동일 원본 UZH event timestamp·pose를 쓴 software world-frame direction-ray projection |
...
| **HOLD** | wire-complete world-coordinate converter RTL 구현과 관측 |
| **HOLD** | converter/world Genus, STA, power, Innovus P&R |
| **HOLD** | retire timestamp를 geometry에 주입한 latency-quality 비교 |
| **HOLD** | predictor, online feedback, metric depth/translation/parallax 보정 |
| **HOLD** | native AER scheduling이 world-projection 정확도를 높인다는 주장 |
| **HOLD** | 다른 UZH sequence와 pan/tilt 강도로의 일반화 |

Ganghee native AER의 별도 PPA/P&R 자료는 이 software world-projection
result authority에 포함되지 않는다. 수치를 쓸 경우에는 원본 report
상대경로, RTL hash, corner/constraint, activity 조건, signoff 한계를
별도로 밝혀야 한다. native PPA를 world-coordinate converter PPA로 합쳐 말하지
않는다.
...
world-coordinate converter/world-grid RTL PPA는 계속 HOLD다.
...
| Junyoung(준영) | UZH raw/cyclemask crosswalk → native outcome → world projection → world grid 통합, 네 시간축 | upstream converter를 이 runner가 재실행하지 않음; retire timing은 geometry가 아닌 sidecar |
...
- “software world-frame direction-ray replay에서 8,420개 WORLD ray와 83개 SENSOR_FIXED
  bypass를 구분했다.”
...
- “upstream converter부터 world projection까지 전체를 재실행했다” → **금지:** converter
  bytes와 cyclemask는 pinned되었지만 converter execution은 재현하지 않았다.
- “event ID를 AER wire로 전송했다” → **금지:** TB-side observational
  identity이다.
- “외부 CAV 시스템 또는 표준 interface와 호환된다” → **금지:** `CAV`는
  팀 내부 식별자이며 software join은 외부 연동이나 wire-complete RTL 증거가 아니다.
- “retire timestamp를 쓰니 품질이 좋아졌다” → **HOLD:** retire timing은
  geometry에 주입하지 않았다.
- “world-coordinate converter/world-grid RTL, memory, PPA를 구현했다” → **HOLD:** 현재는
  software ray/grid mapping이다.
- “metric 3D world point까지 변환했다” → **금지:** depth·translation이 없는
  world-frame direction-ray 투영이다.
- “AER scheduling이 world-projection accuracy를 개선했다” → **HOLD:** 세 view의
  geometry가 의도적으로 같다.
...
3. 전체 흐름 — UZH/cyclemask → sealed native observation → observational
   join → software world projection → world grid.
4. 네 시간축 — sensor ns / 1 ms workload bin / 6.5 ns projection cycle /
   native retire cycle.
5. 결과 — 8,503 join, 8,420 WORLD, 83 bypass, latency histogram, 821 cells.
6. 증거 경계 — PASS/HOLD 표와 speaker-safe claims.
7. 후속 — wire-complete world-coordinate converter RTL, latency-quality experiment, PPA,
   predictor/feedback.

## 9. Committed evidence index

- [1차 상태 문서](../../../docs/REDRED_CLUSTER2_CAV_1ST_ROUND_STATUS_20260824.txt)
- [발표 claim evidence matrix](cluster2_cav_polarity_v1_integration_evidence_matrix_20260826.md)
