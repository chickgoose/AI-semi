...
CANONICAL_COMMIT = "4ce4836fab1309d3468db8e660d2da9af371f784"
CANONICAL_BINDING_SHA256 = "60684f7bfafc5e05f2a577dc5625e3e3ac5c818e494b8f03ee9f711daf9b8c2c"
...
def parse_design(value: Any, where: str) -> dict[str, Any]:
    ...
    result = {
        ...
        for key in ("top", "primary_clock", "reset", "transfer_scope",
                    ...
                    "scope_drain_port", "rx_pending_port")
    }
...
class XmlDesign:
    ...
    def validate_drain_policy(self, design: dict[str, Any],
        ...
        if top_drain_rhs is None or not self.true_requires(top_drain_rhs, scope_net, True):
            raise ContractError(
        ...
        for state_net in (valid_net, pending_net):
            if not self.true_requires(scope_drain_rhs, state_net, False):
                raise ContractError(
