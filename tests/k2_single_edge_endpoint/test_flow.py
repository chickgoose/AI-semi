    ...
    def test_actual_innovus_multiphase_log_and_completion_mutations_fail_closed(self):
        ...
        mutations = {
            ...
            "structured-fatal": actual_shape.replace(
                f"{marker}\n*** Message Summary: 365",
                f"**FATAL: failed\n{marker}\n*** Message Summary: 365", 1),
            "command-echo-error-fatal": actual_shape.replace(
                'innovus 1> puts "' + marker + '"\n',
                'innovus 1> puts "ERROR FATAL ' + marker + '"\n', 1),
            "native-error-on-prompt": actual_shape.replace(
                'innovus 1> puts "' + marker + '"\n',
                'innovus 1> **ERROR: native failure\n', 1),
            "indented-error-limit": actual_shape.replace(
                "Error Limit = 1000; Warning Limit = 1000\n",
                "  Error Limit = 1000; Warning Limit = 1000\n", 1),
            "duplicate-marker": actual_shape.replace(
                f"{marker}\n*** Message Summary: 365",
        ...
        for name, mutation in mutations.items():
            self.assertNotEqual(mutation, actual_shape, name)
            with self.subTest(name=name), self.assertRaisesRegex(
                    flow.FlowError, "zero-error"):
...
class SelfContainedActualShapeTests(unittest.TestCase):
    def setUp(self):
        self.fixture = Fixture()
        self.root = self.fixture.root / "attempt"
        self.contract = self.fixture.contract
        self.top = self.contract["candidates"]["a2"]["top"]
    ...
    def payload(self, relative: str) -> bytes:
        setup = (f"# Generated by: Cadence Innovus {self.innovus_version}\n"
                 f"# Design: {self.top}\n"
                 "# Command: report_timing -view se_setup_view -check_type setup "
                 "-max_paths 50  > \"$output/reports/setup_timing.rpt\"\n"
                 "Path 1: MET Setup Check with Pin u_setup/CK\n"
                 "= Slack Time 0.120\n"
                 "Path 2: MET Late External Delay Assertion\n"
                 "= Slack Time 0.200\n" + flow.report_context_line(
                     "Innovus", self.innovus_version, self.top,
                     "setup_timing", "postroute") + "\n").encode()
        hold = (f"# Generated by: Cadence Innovus {self.innovus_version}\n"
                f"# Design: {self.top}\n"
                "# Command: report_timing -view se_hold_view -check_type hold "
                "-max_paths 50  > \"$output/reports/hold_timing.rpt\"\n"
                "Path 1: MET Hold Check with Pin u_hold/CK\n"
                "= Slack Time 0.040\n"
                "Path 2: MET Early External Delay Assertion\n"
                "= Slack Time 0.090\n" + flow.report_context_line(
                    "Innovus", self.innovus_version, self.top,
                    "hold_timing", "postroute") + "\n").encode()
        genus_timing = (
            f"Generated by: Genus(TM) Synthesis Solution {self.genus_version}\n"
            f"Module: {self.top}\n"
            "Path 1: MET (120 ps) Setup Check with Pin u_setup/CK->D\n"
            "Slack:= 120\n"
            "Path 2: MET (200 ps) Late External Delay Assertion at pin out_o\n"
            "Slack:= 200\n").encode()
        dynamic = {
            "genus/tool.log": EvidenceTests._content(self, "genus_log"),
            "innovus/tool.log": EvidenceTests._content(self, "innovus_log"),
            "innovus/reports/setup_timing.rpt": setup,
            "innovus/reports/hold_timing.rpt": hold,
            "innovus/reports/setup_timing.machine": (
                b"schema=k2_single_edge_timing_summary_v1\nview=se_setup_view\n"
                b"check=setup\npath_count=2\nviolation_count=0\nwns=0.12\ntns=0.0\n"),
            "innovus/reports/hold_timing.machine": (
                b"schema=k2_single_edge_timing_summary_v1\nview=se_hold_view\n"
                b"check=hold\npath_count=2\nviolation_count=0\nwns=0.04\ntns=0.0\n"),
            "genus/reports/timing.rpt": genus_timing,
        }
        if relative in dynamic:
            return dynamic[relative]
        role_by_path = {
            f"genus/netlist/{self.top}.mapped.sdc": "mapped_sdc",
            f"genus/netlist/{self.top}.mapped.v": "mapped_netlist",
            f"genus/netlist/{self.top}.mapped.sdf": "mapped_sdf",
            f"innovus/netlist/{self.top}.postroute.v": "postroute_netlist",
            f"innovus/netlist/{self.top}.postroute.sdf": "postroute_sdf",
            f"innovus/netlist/{self.top}.postroute.spef": "postroute_spef",
            "genus/reports/check_design.rpt": "genus_check_design",
            "genus/reports/timing_intent.rpt": "genus_timing_intent",
            "genus/reports/qor.rpt": "genus_qor",
            "genus/reports/power_vectorless_screening.rpt": "genus_power",
            "genus/reports/clocks.rpt": "genus_clocks",
            "genus/reports/area.rpt": "genus_area",
            "innovus/reports/check_timing.rpt": "check_timing",
            "innovus/reports/drc.rpt": "drc",
            "innovus/reports/antenna.rpt": "antenna",
            "innovus/reports/connectivity.rpt": "connectivity",
            "innovus/reports/pg_connectivity.rpt": "pg_connectivity",
            "innovus/reports/area.rpt": "postroute_area",
            "innovus/reports/power_vectorless_screening.rpt": "postroute_power",
            "innovus/reports/check_design_pre_place.rpt": "check_design_pre_place",
            "innovus/reports/check_place.rpt": "check_place",
            "innovus/reports/check_design_post_route.rpt": "check_design_post_route",
            "innovus/reports/route.rpt": "route",
        }
        return EvidenceTests._content(self, role_by_path[relative])
    ...
    def test_actual_check_timing_inventory_mutations_fail_closed(self):
        ...
        text = raw.decode()
        mutations = {
            "wrong-count": text.replace(
                "| no_drive | No drive assertion | 18 |",
                "| no_drive | No drive assertion | 17 |", 1),
            "foreign-pin": text.replace(
                "| rst_i | No drive assertion | se_setup_view |",
                "| foreign_i | No drive assertion | se_setup_view |", 1),
        ...
        for name, mutation in mutations.items():
            self.assertNotEqual(mutation, text, name)
    ...
    def test_actual_native_identity_and_total_mutations_fail_closed(self):
        ...
        cases = (
            ...
            (flow.validate_innovus_power,
             power.replace("Total Power: 0.002", "Total Power: 9.002", 1).encode(),
             (self.top, self.innovus_version)),
            (flow.validate_innovus_power,
            ...
            (flow.validate_genus_power,
             genus_power.replace("0.000013 100.00%",
                                 "9.000013 100.00%", 1).encode(),
             (self.top,)),
        )
        for validator, payload, args in cases:
            self.assertNotEqual(payload, {
                flow.validate_innovus_area: area.encode(),
                flow.validate_innovus_power: power.encode(),
                flow.validate_genus_check_design: check_design.encode(),
                flow.validate_genus_power: genus_power.encode(),
            }[validator])
