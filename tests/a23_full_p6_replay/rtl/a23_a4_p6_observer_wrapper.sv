...
// Observation-only normalization of the actual A4 scheduler + P6 top.
module a23_a4_p6_observer_wrapper (
  ...
  a4_paired_cortical_column_k2_p6_top dut (
    ...
    .source_ready_o(owner_source_ready), .bundle_valid_o(bundle_valid),
    .bundle_ready_o(bundle_ready), .bundle_commit_o(bundle_commit),
    .grant_count_o(grant_count), .grant_addr0_o(grant_addr0),
  ...
  // Deliberately ignore owner_source_ready: acceptance is reconstructed only
  // from the public whole-bundle commit, count, and ordered addresses.
  always_comb begin
    source_accept_o = '0;
