...
class Stage3New108AdapterTests(unittest.TestCase):
    ...
    def test_projection_rejects_a_non_atomic_warmup_query_boundary(self):
        bundle = self._build()
        window = bundle.neutral_registry[0]
        events = bundle.event_streams[window.window_id]
        warmup = next(event for event in reversed(events) if not event.is_query)
        query = next(event for event in events if event.is_query)
        boundary = window.query_start_ns_inclusive

        def retime(source_event, timestamp_ns):
            return replace(
                source_event,
                ...
                event_content_sha256=canonical_event_content_sha256(
                    ...
                    source_event.is_query,
                    source_event.sensor_ray,
                    source_event.causal_pose_source_index,
                    source_event.transform_guard_valid,
                ),
            )
        ...
        with mock.patch.object(
            adapter_module,
            "_validate_cycle_atomic_boundary",
            side_effect=adapter_module.Stage3New108AdapterError(
            ...
            with self.assertRaisesRegex(
                ...
                self._build()
        self.assertGreater(boundary_check.call_count, 0)
