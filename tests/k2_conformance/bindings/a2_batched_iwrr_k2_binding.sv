...
// Owner-side name shim for A2's combinational-latency K2 candidate.
module k2_candidate_binding (
  ...
  a2_batched_iwrr_k2 owner (
    .clk(clk), .rst(rst), .req(source_pending),
    .grant_count(grant_count), .grant_addr0(grant_addr0),
    .grant_addr1(grant_addr1), .grant_bitmap(unused_grant_bitmap),
    .bundle_ready(bundle_ready), .drain_idle(drain_idle)
  );
endmodule
